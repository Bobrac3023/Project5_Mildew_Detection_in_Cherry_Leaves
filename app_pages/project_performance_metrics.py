...
def project_performance_metrics():
    ...
    the test set.In our plot, the model behaviour is **overfitting**.
    We can explain this because both loss and accuracy plots for training
    and validation data overshooting per epoch, and the validation accuracy
    does not progress with it.As a result, we would see a gap between the
    training and validation accuracy lines.Generally, overfitting is a more
    common phenomenon in neural networks. We can reduce it by tuning our model
    hyperparameters """)
    ...
    training_accuracy_path = os.path.join(model_outputs_dir,
                                          "model_training_acc.png")
    training_loss_path = os.path.join(model_outputs_dir,
                                      "model_training_losses.png")

    if os.path.exists(training_accuracy_path) and
    os.path.exists(training_loss_path):
